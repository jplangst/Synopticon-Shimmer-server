...
namespace ShimmerAPI
{
    public partial class Control : Form
    {
        ...
        public ShimmerSDBT32Feet ShimmerDevice = new ShimmerSDBT32Feet("Shimmer", "");
        //Plot
        ...
        //VRPN Stuffs
        //Connection VRPNConnection = null;
        //TrackerServer VRPNServer = null;
        int VRPNPort = 6789;
        SortedDictionary<string, string> ShimmerPort;
        ...
        private void ControlForm_Load(object sender, EventArgs e)
        {
            ...
            // btsd changes1
            ShimmerDevice = new ShimmerSDBT32Feet("Shimmer", ComPort);
            ...
            ShimmerPort = Util.GetAllShimmerID();
            foreach (var ShimmerName in ShimmerPort.Keys)
                comboBoxComPorts.Items.Add(ShimmerName);
        ...
        private void configureToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (ShimmerDevice.GetShimmerVersion() == (int)Shimmer.ShimmerVersion.SHIMMER3)
            {
                if ((((enabledSensors & 0xFFFFFF) & (int)Shimmer.SensorBitmapShimmer3.SENSOR_INT_A1) > 0)
                    || (((enabledSensors & 0xFFFFFF) & (int)Shimmer.SensorBitmapShimmer3.SENSOR_INT_A12) > 0)
                    || (((enabledSensors & 0xFFFFFF) & (int)Shimmer.SensorBitmapShimmer3.SENSOR_INT_A13) > 0)
                    || (((enabledSensors & 0xFFFFFF) & (int)Shimmer.SensorBitmapShimmer3.SENSOR_INT_A14) > 0))
                {
                    EnablePPGtoHRConversion = true;
                }
            }
        ...
        {
            //ShimmerDevice.SetShimmerName();
            RemoveAllTextBox();
            Connect();

       
        }
        private string ShimmerDeviceName = "";
        public void Connect()
        {
            ...
            ShimmerDeviceName = comboBoxComPorts.Text;
            if (ShimmerPort.ContainsKey(ShimmerDeviceName))
            {
                ShimmerDevice.SetAddress(ShimmerPort[ShimmerDeviceName]);
                ShimmerDevice.SetShimmerName(ShimmerDeviceName);
            }
        ...
        public void HandleEvent(object sender, EventArgs args)
        {
            ...
            Shimmer32Feet shimmer = sender as Shimmer32Feet;

            switch (indicator)
            {
                case (int)Shimmer.ShimmerIdentifier.MSG_IDENTIFIER_STATE_CHANGE:

                    System.Diagnostics.Debug.Write(shimmer.GetDeviceName() + " State = " + shimmer.GetStateString() + System.Environment.NewLine);
                    int state = (int)eventArgs.getObject();
                    if (state == (int)Shimmer.SHIMMER_STATE_CONNECTED)
                    {
                        AppendTextBox("Connected");
                        ChangeStatusLabel("Connected to " + ShimmerDevice.GetShimmerName() + ". Firmware Version: " + ShimmerDevice.GetFirmwareVersionFullName());
                ...
                case (int)Shimmer.ShimmerIdentifier.MSG_IDENTIFIER_NOTIFICATION_MESSAGE:
                    ...
                    System.Diagnostics.Debug.Write(shimmer.GetDeviceName() + message + System.Environment.NewLine);
                    //Message BOX
                ...
                case (int)Shimmer.ShimmerIdentifier.MSG_IDENTIFIER_DATA_PACKET:
                    ...
                    */
                    ...
                    double[] VRPNData = new double[2];
                    VRPNData[0] = -1;
                    VRPNData[1] = -1;
                    var GSRIndex = names.FindLastIndex(a => a == "GSR");
                    if (GSRIndex > 0)
                        VRPNData[0] = Math.Truncate(data[GSRIndex] * 100) / 100;
                    var HRIndex = names.FindLastIndex(a => a == "Heart Rate PPG");
                    if (HRIndex > 0)
                        VRPNData[1] = Math.Truncate(data[HRIndex] * 100) / 100;
                    //Thread VRPNThread = new Thread(() => StreamDataToVRPN(VRPNData));
                    //VRPNThread.Start();
                    //StreamDataToVRPN(VRPNData);
                    if(IsBroadcastingToWAMP)
                        StreamDataToWAMP(VRPNData);
                    break;
        ...
        //#region VRPN Server Stuff
        ////Should be called periodically
        //private void StreamDataToVRPN(double[] Data)
        //{
        //    //SortedDictionary<String, double> data;
        //    System.Windows.Media.Media3D.Point3D Pos = new System.Windows.Media.Media3D.Point3D(Data[0], Data[1], 0);
        //    System.Windows.Media.Media3D.Quaternion Quat = new System.Windows.Media.Media3D.Quaternion();
        //    VRPNServer.ReportPose(0, DateTime.Now, Pos, Quat);
        ...
        //            VRPNServer.Update();
        //        }
        //        if (VRPNConnection != null)
        //        {
        //            VRPNConnection.Update();
        //        }
        //    }
        //    catch(VrpnException e)
        //    {
        ...
        //private void CreateVRPNServer()
        //{
        //    if (VRPNConnection == null)
        //    {
        //        VRPNPort = int.Parse(wamp_router_adress.Text);
        //        VRPNConnection = Connection.CreateServerConnection(VRPNPort);
        //    }

            
        //    if (VRPNServer == null)
        //        VRPNServer = new TrackerServer(comboBoxComPorts.Text, VRPNConnection);

        //    VRPNServer.Update();
        //    VRPNConnection.Update();
        //}

        //private void CloseVRPNServer()
        //{
        //    if (VRPNServer != null)
        //    {
        //        VRPNServer.Dispose();
        ...
        #region WAMPClient
        private string WAMPServerAddress = "ws://127.0.0.1:8080/ws";
        private string WAMPRealm = "realm1";
        private string WAMPTopic = "ShimmerDataSample";
        private IWampChannel channel = null;
        private IArgumentsService proxy = null;
        private ISubject<Tuple<string, double[]>> WAMPSubject = null;
        ...
        private void CreateWAMPClient()
        {
            ...
            WAMPServerAddress = "ws://" + wamp_router_adress.Text + ":" + textBoxWAMPPort.Text + "/ws";
            channel = channelFactory.CreateJsonChannel(WAMPServerAddress, WAMPRealm);
            counter = 0;
            ConnectToRouter();
        }
        ...
        private void ConnectToRouter()
        {
            if (channel != null)
            {
                try
                {
                    ...
                    WAMPSubject =
                        realm.Services.GetSubject<Tuple<string, double[]>>(WAMPTopic);


                    ICalleeProxyInterceptor callOptions = new CalleeProxyInterceptor(new CallOptions() { DiscloseMe = true });

                    proxy = channel.RealmProxy.Services.GetCalleeProxy<IArgumentsService>(callOptions);
                    try
                    {
                        object[] info = new object[2] { ShimmerDeviceName, "shimmer" };
        ...
        private void StreamDataToWAMP(double[] Data)
        {
            if(WAMPSubject != null)
            {
                try
                {
                    WAMPSubject.OnNext(new Tuple<string, double[]>(ShimmerDeviceName, Data));
                }
                ...
                {
                    //general exception: skip this frame
                }
            }           
        }

        private void CloseWAMPClient()
        {
            if(channel != null)
            {
                try
        ...
        private void StreamShimmerData()
        {
            int enabledSensors = ShimmerDevice.GetEnabledSensors();
            if (ShimmerDevice.GetShimmerVersion() == (int)Shimmer.ShimmerVersion.SHIMMER3)
            {
    ...
    public interface IArgumentsService
    {
        ...
        void RegisterPublisher(string evt, object info);
    }
